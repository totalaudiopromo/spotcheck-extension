...
// Feature limits by tier. Free is intentionally narrow -- 2 checks a day is
// enough to land the "is it bot?" hook on a real playlist; everything past
// that is the upgrade.
const LIMITS = {
  [TIERS.FREE]: {
    dailyChecks: 1,
    ...
    tracking: false,
    export: false,
    botScore: 'preview', // headline score visible, factors paywalled
    api: false,
  },
  [TIERS.PREMIUM]: {
...
export const Premium = {
  ...
  /**
   * Get checkout URL for a plan. The backend GET handler at api.spotcheck.cc
   * creates a Stripe Checkout Session and 302s straight to Stripe -- no
   * intermediate page on spotcheck.cc, no client-side fetch needed.
   * @param {string} plan - 'premium' or 'pro'
   * @returns {string}
   */
  getCheckoutUrl(plan) {
    return `https://api.spotcheck.cc/api/checkout?plan=${encodeURIComponent(plan)}`;
  },

  /**
   * Get customer portal URL. spotcheck.cc/account is a static page that
   * surfaces "manage subscription" -- Stripe billing portal session is
   * minted there per-user via /api/subscription/portal.
   * @returns {string}
   */
  getPortalUrl() {
    return 'https://spotcheck.cc/account';
  },
